...
export const fetchAllProducts = function () {
   return async function (dispatch) {
      try {
         const fetchAllProducts = await axios.get(
   ...
   return async function (dispatch) {
      try {
         const selectedProduct = await axios.get(
            `http://localhost:8000/get-selected-product/${id}`,
            headers
         );
   ...
   return async function (dispatch) {
      try {
         const update = await axios.patch(
            'http://localhost:8000/update-product-data',
            data,
            headers
         );
   ...
   return async function (dispatch) {
      try {
         const productDelete = await axios.delete(
            `http://localhost:8000/delete-selected-product/${data}`,
            headers
         );
...
export const addToCardItems = function (data, productData) {
   return async function (dispatch) {
      try {
         const addProductIntoCart = await axios.post(
            `http://localhost:8000/add-product-into-cart/${data}`,
            headers
         );
...
export const fetchAllCartProduct = function () {
   return async function (dispatch) {
      try {
         const cardProducts = await axios.get(
            'http://localhost:8000/get-all-cards-products',
            headers
         );
